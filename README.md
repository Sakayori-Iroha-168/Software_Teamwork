...
当前 `deploy/docker-compose.yml` 是本地/演示联调基线，不是生产部署基线。生产或准生产单机 Compose 基线见：

- [生产/准生产部署基线](deploy/production-baseline.md)
- `deploy/docker-compose.production.yml`
- `deploy/nginx/production.conf`
- `deploy/.env.production.example`

该基线独立于本地 demo seed，使用发布镜像、单一 ingress 公共入口、持久化卷、生产环境变量模板、secret 注入说明、健康检查和回滚说明。真实云环境部署、DNS/TLS、受保护发布流水线和 #125 完整跨服务 smoke 仍需后续任务补齐。
