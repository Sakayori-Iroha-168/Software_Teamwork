...
| MinIO 对象 | `services/document/internal/platform` | 保存模板、素材和导出文件；公开 API 不返回 object key |
...
- 不实现前端页面或管理后台页面。
- 不实现模型服务配置管理。
- 不在 gateway 中实现报告生成业务规则、SQL、MinIO 操作或 AI 编排。
...
- `internal/http` 只做请求解析、响应映射和错误转换，不写 SQL、不直接调用 MinIO。
- `internal/service` 负责状态流转、事务边界、任务编排和审计日志。
- `internal/repository` 使用显式 SQL 和参数化查询。
- `internal/platform` 封装 MinIO、模型服务、gateway client 等外部依赖。
...
| 2 | `report_templates` | 模板元数据和 MinIO 模板文件引用 |
| 3 | `report_materials` | 素材元数据和 MinIO 素材文件引用 |
...
| 12 | `report_files` | 导出文件元数据和 MinIO object key |
...
- `file_object_key` 和 `object_key` 只在服务内部使用，不返回给公开 API。
...
- 内部错误不得包含 SQL、MinIO object key、prompt、内部 URL 或密钥。
...
3. Platform MinIO 生成 object key 并上传文件。
4. Repository 保存元数据、文件名、文件大小和 object key。
5. 返回模板或素材元数据，不返回 object key。
...
3. 读取当前报告、章节和模板样式。
4. 生成 DOCX，上传 MinIO。
5. 保存 `report_files.object_key`、`file_name`、`file_size`、`status`。
6. 返回 `reportFileId`、`status`、`contentPath`，不返回 object key。
...
- `parameter_summary_json` 和 `metadata_json` 必须脱敏，不保存密钥、完整 prompt、MinIO object key 或完整文档内容。
...
- MCP 输出不返回 MinIO object key、内部 URL、模型密钥或完整 prompt。
...
- 创建 `internal/config`，校验 PostgreSQL、MinIO、服务监听地址等配置。
- 创建健康检查和基础路由。
- 创建 Dockerfile。
...
- 添加 `report_files` 表。
- 实现 DOCX 文件创建任务框架。
- 实现 MinIO 上传、文件元数据保存和 content 接口。
...
| Platform | MinIO object key 不外露、上传失败处理 |
