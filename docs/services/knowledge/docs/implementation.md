...
| `knowledge-queries` 检索 | `services/knowledge/internal/service/retrieval.go`、`internal/http/server.go`、Gateway proxy route | `docs/services/knowledge/docs/api-contract.md`、`docs/services/gateway/api/public.openapi.yaml` | service retrieval tests、`TestKnowledgeQueryContractWithSeededRepositoryAndFakeVector`、`TestKnowledgeQueriesRouteProxiesToKnowledge` | 基于 embedder + vector index 搜索，回 PostgreSQL hydrate chunks/documents，过滤未 ready/不可见文档，支持 tags、metadata filter、可选 AI Gateway rerank 和 local no-op rerank fallback。`topK` 和 `rerankTopN` 受最大 100 约束，rerank 结果 slice/map allocation 按有效上限分配；#414 新增的 chunk feedback ranking signal 尚待实现。 |
...
| Chunk feedback ranking 尚未实现 | #414 / `docs/services/knowledge/api/internal.openapi.yaml` | retrieval / QA integration | 需补 internal `POST /internal/v1/chunk-feedback` handler、事件/聚合表、幂等累计、`knowledge-queries` 排名算子和 `feedbackApplied` trace 测试。 |
