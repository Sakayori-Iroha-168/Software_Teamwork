...
| 2026-07-01 | Codex #343 branch | `develop@96b5ad8f` + 本分支改动 | 新增 QA active path schema-level contract tests，覆盖当时 25 个 QA-owned operations、SSE 唯一路径、ErrorResponse envelope、分页 schema、QA internal `$ref` drift 和默认 proxy namespace/query 映射；#414 将 QA active operations 扩展到 message feedback，route/proxy tests 需随实现补齐；真实 Redis/downstream smoke 仍待补齐。 |
