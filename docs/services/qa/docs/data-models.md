...
QAMessage 1 ── 0..1 MessageFeedback per external_user_id
...
### 10.2 MessageFeedback

表名建议：`message_feedback`

用户对助手消息的点赞/点踩状态。QA 只保存 message-level 反馈事实和与当前用户的幂等状态；chunk-level 权重、衰减和排名使用归 Knowledge 拥有。

| 字段 | 类型建议 | 说明 |
| --- | --- | --- |
| `id` | uuid/string | 反馈记录 ID。 |
| `message_id` | uuid/string | 被反馈的助手消息 ID。 |
| `conversation_id` | uuid/string | 冗余会话 ID，用于 owner 校验和排查。 |
| `external_user_id` | string | 当前反馈用户 ID。 |
| `vote` | string | `up`、`down` 或 `none`。`none` 表示清除已有反馈。 |
| `citation_chunk_ids_snapshot` | jsonb | 写入时从 `citations.external_chunk_id` 归因得到的 chunk ID 快照；仅供审计/重放，不作为公开字段返回。 |
| `knowledge_feedback_status` | string | `pending`、`applied`、`skipped` 或 `failed`。 |
| `knowledge_feedback_error_code` | string | Knowledge 内部调用失败时的脱敏错误码。 |
| `created_at` | datetime | 创建时间。 |
| `updated_at` | datetime | 更新时间。 |
...
- FK：`message_id -> messages.id`。
- FK：`conversation_id -> conversations.id`。
- 唯一约束：`(message_id, external_user_id)`，同一用户对同一助手消息只有一个当前反馈。
- `vote IN ('up', 'down', 'none')`。
- 只允许对当前用户可访问会话内的助手消息写入反馈；对用户消息、system 消息或未完成消息返回 `validation_error` 或 `conflict`。
- 前端不得提交 chunk ID；QA 必须从已保存的 `citations` 读取 `external_chunk_id`，再向 Knowledge 内部 chunk feedback 资源上报。
- Knowledge 内部调用失败不得暴露内部 URL、payload 或 storage identifier；公开响应仍只返回 message-level feedback 状态，必要时返回统一 `dependency_error`。
...
| `vote` | `vote` |
| `citation_chunk_ids_snapshot` | 不暴露 |
| `knowledge_feedback_status` | 不暴露 |
...
| `idx_message_feedback_user` on `(external_user_id, updated_at DESC)` | 用户反馈历史和审计。 |
| `idx_message_feedback_status` on `(knowledge_feedback_status, updated_at)` | 重试或排查 Knowledge feedback 上报状态。 |
...
| `uniq_message_feedback_user` on `message_feedback(message_id, external_user_id)` | 保证同一用户对同一消息的反馈幂等 upsert。 |
...
```text
gateway forwards POST /api/v1/messages/{messageId}/feedback
qa verifies current user owns the conversation or has feedback permission
qa verifies message is an assistant message in completed state
qa upserts message_feedback(message_id, external_user_id, vote)
qa reads citations for message_id and extracts non-empty external_chunk_id values
qa calls knowledge /internal/v1/chunk-feedback with chunk feedback deltas
qa records knowledge_feedback_status as applied / skipped / failed
qa returns message-level feedback through gateway
```

实现约束：

- `vote=none` 清除当前用户对该消息的反馈，并向 Knowledge 发送反向 delta 或清除语义，具体权重算法由 Knowledge 拥有。
- 如果消息没有 citations 或没有可归因的 `external_chunk_id`，QA 应保存 message-level 反馈并将 `knowledge_feedback_status` 记为 `skipped`。
- QA 不保存或计算 chunk 的最终 feedback score；它只把消息反馈归因为 Knowledge 可消费的信号。
