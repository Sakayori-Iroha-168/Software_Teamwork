...
| 消息反馈 | 保存用户对助手消息的点赞/点踩反馈事件，根据该消息的引用快照归因到 `external_chunk_id`，并通过 Knowledge 内部接口上报 chunk feedback 信号。 |
...
qa service
   ...
   +--> knowledge /internal/v1/chunk-feedback for chunk feedback weight updates
```
...
- `messages/{messageId}/feedback`：当前用户对助手消息的反馈。前端只提交 message-level `vote`，QA 从已保存的 citations 中读取 chunk IDs 并向 Knowledge 上报 feedback 信号。
...
| 消息反馈 | `message_feedback` 或 `message_feedback_events` | 保存当前用户对助手消息的 `up` / `down` / `none` 反馈；chunk 归因只读取 QA 已保存的 citations，chunk feedback 权重归 Knowledge 持久化和使用。 |
