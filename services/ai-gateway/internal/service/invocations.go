...
func (s *Service) CreateEmbeddings(ctx context.Context, req RequestContext, input EmbeddingInput) (EmbeddingResponse, error) {
	...
	invocation := s.newInvocation(req, OperationEmbedding, profile, strings.TrimSpace(input.Model), startedAt)
...
func (s *Service) CreateReranking(ctx context.Context, req RequestContext, input RerankingInput) (RerankingResponse, error) {
	...
	startedAt := time.Now().UTC()
	invocation := s.newInvocation(req, OperationReranking, profile, strings.TrimSpace(input.Model), startedAt)
...
func (s *Service) resolveInvocationProfile(ctx context.Context, profileID string, purpose Purpose) (ModelProfile, ProviderCredential, error) {
	...
	if strings.TrimSpace(profileID) != "" {
		...
		enabled := true
		profiles, err := s.ListModelProfiles(ctx, ListModelProfilesFilter{Purpose: &purpose, Enabled: &enabled})
		if err != nil {
			return ModelProfile{}, ProviderCredential{}, err
		}
		...
		if profile.ID == "" {
			return ModelProfile{}, ProviderCredential{}, NotFoundError("default model profile not found", ErrNotFound)
		}
	}
...
func (s *Service) newInvocation(req RequestContext, operation Operation, profile ModelProfile, model string, startedAt time.Time) ProviderInvocation {
...
func (s *Service) finishInvocation(ctx context.Context, invocation ProviderInvocation, metadata ProviderCallMetadata, usage *TokenUsage, callErr error) error {
	...
	invocation.DurationMS = int(finishedAt.Sub(invocation.CreatedAt).Milliseconds())
	...
	if callErr != nil {
		...
		invocation.NormalizedErrorCode = &appErr.Code
	...
	if err := s.repo.RecordProviderInvocation(ctx, invocation); err != nil {
