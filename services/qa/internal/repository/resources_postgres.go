...
const citationSelect = `SELECT ci.id::text,ci.message_id::text,ci.citation_no,COALESCE(ci.external_doc_id,''),ci.doc_name,COALESCE(ci.external_kb_id,''),COALESCE(ci.external_chunk_id,''),COALESCE(ci.section_path,''),COALESCE(ci.quote_text,''),COALESCE(ci.context,''),ci.page_number,ci.score,ci.rerank_score,COALESCE(ci.chunk_type,''),ci.metadata,COALESCE(ci.content_preview,''),ci.is_source_available,COALESCE(ci.source_unavailable_reason,'') FROM citations ci JOIN messages m ON m.id=ci.message_id JOIN conversations c ON c.id=m.conversation_id`
...
func scanCitation(row rowScanner) (service.Citation, error) {
	...
	var sourceUnavailReason sql.NullString
	err := row.Scan(&item.ID, &item.MessageID, &item.CitationNo, &item.DocumentID, &item.DocumentName, &item.KnowledgeBaseID, &item.ChunkID, &item.SectionPath, &item.Text, &item.Context, &page, &score, &rerank, &item.ChunkType, &metadata, &contentPreview, &isSourceAvail, &sourceUnavailReason)
	...
	item.IsSourceAvailable = isSourceAvail
	item.Source = &service.CitationSource{Available: isSourceAvail}
	if isSourceAvail && item.DocumentID != "" {
		item.Source.DownloadEndpoint = "/api/v1/documents/" + item.DocumentID + "/content"
	} else if !isSourceAvail {
		if sourceUnavailReason.Valid {
			item.Source.Reason = sourceUnavailReason.String
		} else {
