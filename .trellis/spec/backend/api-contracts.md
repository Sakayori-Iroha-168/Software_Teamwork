...
## Scenario: Document Report Workflow APIs

### 1. Scope / Trigger

- Trigger: adding or changing Document Service report draft, outline, section,
  job, retry attempt, event, generated file, report statistic, operation log, or
  settings behavior.
- Applies to `services/document/internal/http`, `services/document/internal/service`,
  `services/document/internal/repository`, `services/document/migrations`, and
  matching active routes in `docs/services/gateway/api/openapi.yaml`.

### 2. Signatures

- Gateway-backed resource routes:
  ...
  - `POST /report-files`
  - `GET /report-files`
  - `GET /report-files/{reportFileId}`
  - `GET /report-files/{reportFileId}/content`
  - `GET /report-statistics/overview`
  - `GET /report-statistics/daily`
  - `GET /report-operation-logs`
  - `GET /report-settings`
  - `PATCH /report-settings`
- Durable tables include `reports`, `report_outlines`, `report_sections`,
  `report_section_versions`, `report_jobs`, `report_job_attempts`,
  `report_events`, `report_files`, `report_operation_logs`, and
  `report_settings`.

### 3. Contracts

- `report_jobs`, `report_job_attempts`, and `report_events` are the durable
  authority for workflow progress, retry history, and user-visible events.
- Retrying a job must create a `report_job_attempts` row and update the parent
  `report_jobs.retry_count` in the same database transaction.
- Retrying a job is allowed only for failed or canceled report jobs. Completed,
  running, pending, or partial-success jobs must return a conflict instead of
  creating another attempt.
- Task and generation creation routes (`POST /reports/{reportId}/jobs`,
  `POST /report-jobs/{jobId}/attempts`, and `POST /report-files`) return
  `202 Accepted` to match their asynchronous workflow contract.
- Generated file metadata may expose only safe display fields such as `id`,
  `filename`, `format`, `fileSize`, `status`, and `contentPath`.
- Generated report files are immutable export snapshots. Downloading an old
  `reportFileId` must return the content captured when that file resource was
  created, not regenerated bytes from the current editable report sections.
- Listing generated files without `reportId` must be admin-only or scoped to
  reports the current user can access. A non-admin caller must never receive
  metadata for another user's report files.
- `GET /report-files/{reportFileId}/content` returns binary DOCX bytes on
  success. Error responses still use the standard `{ error }` envelope.
- Global report settings, global statistics, and report operation logs are
  administrative surfaces. Require an admin-level role or explicitly filter the
  result set by the current user before returning data.
- Public responses must not expose `file_ref`, File Service IDs, object keys,
  buckets, internal URLs, prompts, provider raw errors, SQL details, API keys,
  or tokens.
...
| --- | --- |
| Missing gateway user context | `401 unauthorized` |
| Invalid report/job/file UUID or unsupported job type | `400 validation_error` |
| Missing report, job, section, or file | `404 not_found` |
| Deleted report or retry limit reached | `409 conflict` |
| Unsupported report file format | `400 validation_error` |
| PostgreSQL or generation dependency failure | `502 dependency_error` |
| DOCX packaging failure | `500 internal_error` |

### 5. Good/Base/Bad Cases

- Good: a report job writes job, attempt, event, generated outline/sections, and
  report workflow state through service-layer orchestration and repository
  methods.
- Base: a synchronous MVP may generate deterministic template-backed content
  while preserving the same durable job/attempt/event contract.
- Bad: retry writes only an attempt row without updating `report_jobs`, or a
  file response exposes File Service internals instead of a safe content path.

### 6. Tests Required

- Handler tests for JSON envelopes, binary DOCX content headers, request id
  propagation, route coverage, and dependency error mapping.
- Service tests for outline/content generation, retry count updates, retry
  limits, file creation, DOCX content build, immutable export snapshots, file
  list ownership scoping, retryable job status checks, and admin-only global
  settings/statistics/log access.
- Repository integration tests, when `DOCUMENT_TEST_DATABASE_URL` is available,
  for job, attempt, event, file, settings, and transaction behavior.
- Frontend typecheck/build after changing generated gateway report workflow
  types or feature wrappers.
...
```text
POST /report-jobs/{jobId}/attempts -> insert attempt only -> parent job retry_count stays stale
```

#### Correct

```text
POST /report-jobs/{jobId}/attempts -> transaction inserts attempt, increments report_jobs.retry_count, records retry event
```
