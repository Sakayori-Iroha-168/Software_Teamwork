...
`Astronia-A` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/Astronia-A/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/Astronia-A/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee Astronia-A` to
...
python ./.trellis/scripts/task.py archive 00-join-astronia-a
```
